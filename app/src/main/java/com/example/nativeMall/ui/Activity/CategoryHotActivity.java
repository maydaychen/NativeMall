...
public class CategoryHotActivity extends InitActivity implements PullLoadMoreRecyclerView.PullLoadMoreListener {
            ...
            public void onNext(JSONObject jsonObject) throws JSONException {
                if (jsonObject.getInt("statusCode") == 1) {
                    ...
                    try {
                        ...
                        if (page != 0) {
                            Toast.makeText(CategoryHotActivity.this, "已经加载完毕！", Toast.LENGTH_LONG).show();
                        }
                    }
                } else {
                    Toast.makeText(CategoryHotActivity.this, jsonObject.getString("result"), Toast.LENGTH_SHORT).show();
                }
            }
...
//        initCategoryGoods();
    ...
    private void initCategoryGoods() {
        ...
        HttpJsonMethod.getInstance().getAttributes(
                new ProgressErrorSubscriber<>(hotOnNext, CategoryHotActivity.this),
                preferences.getString("access_token", ""), sortType, "ishot:1", page, 10, sign, time);
    }
